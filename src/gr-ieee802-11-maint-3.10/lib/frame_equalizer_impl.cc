...
frame_equalizer_impl::frame_equalizer_impl(
    ...
    : gr::block("frame_equalizer",
                gr::io_signature::make2(2, 2, 64 * sizeof(gr_complex), sizeof(gr_complex)),
      ...
      //TODO
      //short_pre_num(d_short_pre_num),
      //short_pre_len(d_short_pre_len)
{
                                       ...
                                       gr_vector_void_star& output_items)
{
    ...
    const gr_complex* in_pre = (const gr_complex*)input_items[1];
    ...
    double offset = 0;
    double short_freq = 0;
    int preamble_num = 288;
    std::vector<gr_complex> pre(preamble_num);
    while ((i < ninput_items[0]) && (o < noutput_items)) {
        ...
        for (auto tag : tags){
            if (flag==0){
                offset = pmt::to_double(tag.value);
                dout<<"offset"<<offset<<std::endl;
            }
            if (flag ==2){
                for(int i=0;i<preamble_num;i++){
                    //dout<<"pre"<<pmt::vector_ref(tag.value, i)<<std::endl;
                    pre[i] = pmt::to_complex(pmt::vector_ref(tag.value, i));
                }
            }
            if (flag ==1){
                short_freq = pmt::to_double(tag.value);
                dout<<"short_offset"<<short_freq<<std::endl;
            }
            flag++;
        }
        //dout<<"tags:"<<tags<<std::endl;
        //, pmt::string_to_symbol("wifi_start")
        //TODO END
        // get_tags_in_window(tags, 0, i, i + 1,pmt::string_to_symbol("wifi_start"));
        // get_tags_in_window(pre_tags, 0, i, i + 1,pmt::string_to_symbol("prembel"));
        // //TODO
    
        // offset = pmt::to_double(tags.front().value);
        // dout<<"offset"<<pmt::to_double(tags.front().value)<<std::endl;


        // for(int i=0;i<256;i++){
        //     //dout<<"pre"<<pmt::vector_ref(pre_tags.front().value, i)<<std::endl;
        //     pre[i] = pmt::to_complex(pmt::vector_ref(pre_tags.front().value, i));
        // }
        // new frame
        if (tags.size()) {
            d_current_symbol = 0;
            ...
            d_er = 0;
            dout<<"freq:"<<d_freq_offset_from_synclong<<std::endl;
        ...
        if (d_current_symbol == 2) {
            ...
            if (decode_signal_field(out + o * 48)) {
                ...
                dict = pmt::dict_add(dict,
                                     pmt::mp("short_freq"),
                                     pmt::from_double(short_freq));
                dict = pmt::dict_add(dict,
                                     pmt::mp("long_freq"),
                                     pmt::from_double(offset));
                dict = pmt::dict_add(dict, pmt::mp("preamble"), pmt::init_c32vector(pre.size(), pre));
                //dict = pmt::dict_add(
                //dict, pmt::mp("csi"), pmt::init_c32vector(ninput_items[1], in_pre));
                //dout<<"Number:"<<ninput_items[1]<<"Preamble"<<pmt::init_c32vector(ninput_items[1], in_pre)<<std::endl;
